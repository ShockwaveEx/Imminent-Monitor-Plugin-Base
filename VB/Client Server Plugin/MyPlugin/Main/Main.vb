...
#Region " GUI "
    ...
    Private Sub AddContextItemWithChildren()
        ...
        Dim ChildItem As New ContextItem() With {.Clicked = AddressOf HandleContextItemClicked, .Name = "Your Feature"} 'Creating child item 'We can add an image if we like using the .Image property
...
#Region " Context Item Clicked Handlers "
    Private Sub HandleContextItemClicked(clients As IServerClient()) 'A Clicked handler must always take an array of clients as a parameter
        ...
        Dim F As New FormMessage(clients(0))
        F.Show()

        FormMessageHandle = F 'We keep an instance of this because we will need to close it in the unload event
    End Sub
